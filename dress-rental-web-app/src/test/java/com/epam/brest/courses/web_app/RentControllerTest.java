...
class RentControllerTest {
    ...
    void shouldReturnDressPage() throws Exception {
        ...
        Mockito.when(rentService.findAllByDate(dateFrom, dateTo)).thenReturn(rents);
    ...
    void shouldReturnDressPageWithErrorIfDateFromIsAfterDateTo() throws Exception {
        ...
        Mockito.when(rentService.findAllByDate(null, null)).thenReturn(rents);
    ...
    void shouldGotoAddRentPage() throws Exception {
        List<DressDto> dresses = Arrays.asList(new DressDto(), new DressDto());
        Mockito.when(dressService.findAllWithNumberOfOrders()).thenReturn(dresses);
        ...
        mockMvc.perform(get(RENTS_ENDPOINT + "/new")).andDo(print())
                ...
                .andExpect(model().attribute("rentDto", isA(RentDto.class)))
    ...
    void shouldGotoEditRentPage() throws Exception {
        ...
        mockMvc.perform(get(RENTS_ENDPOINT + "/" + id)).andDo(print())
                ...
                .andExpect(content().contentType("text/html;charset=UTF-8"))
                .andExpect(view().name("rent"))
                .andExpect(model().attribute("rentDto", hasProperty("rentId", is(rent.getRentId()))))
                .andExpect(model().attribute("rentDto", hasProperty("client", is(rent.getClient()))))
                .andExpect(model().attribute("rentDto", hasProperty("rentDate", is(rent.getRentDate()))))
                .andExpect(model().attribute("rentDto", hasProperty("dressName", is(rent.getDressName()))));
    }
    ...
    void shouldUpdateRentAfterEdit() throws Exception {
        ...
        rent.setRentDate(LocalDate.now());
        rent.setDressName("Dress");
        Mockito.when(rentService.createOrUpdate(rent)).thenReturn(1);
        ...
        mockMvc.perform(post(RENTS_ENDPOINT)
                ...
                .param("dressName", rent.getDressName())
