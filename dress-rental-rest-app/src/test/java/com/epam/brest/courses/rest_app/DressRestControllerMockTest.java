...
class DressRestControllerMockTest {
    ...
    void shouldFindAll() throws Exception {
        DressDto firstDress = new DressDto();
        firstDress.setDressId(1);
        firstDress.setDressName("First dress");
        firstDress.setNumberOfOrders(2);
        DressDto secondDress = new DressDto();
        secondDress.setDressId(2);
        secondDress.setDressName("Second dress");
        secondDress.setNumberOfOrders(1);
        List<DressDto> dresses = Arrays.asList(firstDress, secondDress);

        when(dressService.findAllWithNumberOfOrders()).thenReturn(dresses);
        ...
        verify(dressService, times(1)).findAllWithNumberOfOrders();
    ...
    void shouldFindByExistingId() throws Exception {
        ...
        dressDto.setDressName("First dress");
        dressDto.setNumberOfOrders(2);

        when(dressService.findById(dressDto.getDressId())).thenReturn(Optional.of(dressDto));

        mockMvc.perform(get(DRESSES_ENDPOINT + "/" + dressDto.getDressId()))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(content().string(mapper.writeValueAsString(dressDto)));

        verify(dressService, times(1)).findById(dressDto.getDressId());
    }
    ...
    void shouldCreateNewDress() throws Exception {
        ...
        when(dressService.createOrUpdate(any())).thenReturn(id);

        mockMvc.perform(post(DRESSES_ENDPOINT)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(dressDto))
        )
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(content().string(mapper.writeValueAsString(id)));

        verify(dressService, times(1)).createOrUpdate(any());
    }

    @Test
    void shouldReturnInternalServerErrorWhenCreateDressWithInvalidData() throws Exception {
        when(dressService.createOrUpdate(any())).thenThrow(new DataIntegrityViolationException("Error"));
    ...
    void shouldReturnUnprocessableEntityWhenCreateExistingDress() throws Exception {
        when(dressService.createOrUpdate(any())).thenThrow(new IllegalArgumentException());
    ...
    void shouldUpdateDress() throws Exception {
        when(dressService.createOrUpdate(any())).thenReturn(1);
