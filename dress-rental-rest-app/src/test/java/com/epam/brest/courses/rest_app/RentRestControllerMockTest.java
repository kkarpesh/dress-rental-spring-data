...
class RentRestControllerMockTest {
    ...
    void shouldFindAll() throws Exception {
        ...
        secondRent.setDressName("Dress2");
        List<RentDto> rents = Arrays.asList(firstRent, secondRent);

        when(rentService.findAllByDate(any(), any())).thenReturn(rents);
        ...
        verify(rentService, times(1)).findAllByDate(any(), any());
    ...
    void shouldCreateNewRent() throws Exception {
        ...
        when(rentService.createOrUpdate(any())).thenReturn(id);
        ...
        verify(rentService, times(1)).createOrUpdate(any());
    }

    @Test
    void shouldReturnInternalServerErrorWhenCreateRentWithInvalidData() throws Exception {
        when(rentService.createOrUpdate(any())).thenThrow(new DataIntegrityViolationException("Error"));
    ...
    void shouldReturnUnprocessableEntityWhenCreateExistingRent() throws Exception {
        when(rentService.createOrUpdate(any())).thenThrow(new IllegalArgumentException());
    ...
    void shouldUpdateRent() throws Exception {
        when(rentService.createOrUpdate(any())).thenReturn(1);
    ...
    void shouldCheckNameAlreadyExists() throws Exception {
        Boolean result = true;
        when(rentService.isDressRented(any())).thenReturn(result);
        ...
        verify(rentService, times(1)).isDressRented(any());
    }
}